...
//        pclog("TandySL OUT %04X %02X\n",addr,val);
	...
	case 0x3d2:
	case 0x3d4:
	case 0x3d6:tandy->crtcreg = val & 0x1f;
		return;
	case 0x3d1:
	case 0x3d3:
	case 0x3d5:
...
//                pclog("now %02x\n", tandy->crtc[tandy->crtcreg]);
		...
		return;
	case 0x3d8:tandy->mode = val;
		return;
	case 0x3d9:tandy->col = val;
		return;
	case 0x3da:tandy->array_index = val & 0x1f;
		break;
	case 0x3de:
...
//        if (addr!=0x3DA) pclog("Tandy IN %04X\n",addr);
	switch (addr) {
	case 0x3d4:return tandy->crtcreg;
	case 0x3d5:return tandy->crtc[tandy->crtcreg];
	case 0x3da:return tandy->stat;
	}
...
//                                printf("Firstline %i\n",firstline);
			...
			for (c = 0; c < 8; c++) {
				if (tandy->array[3] & 4) {
					...
					if (tandy->mode & 1)
						((uint32_t *)buffer32->line[tandy->displine])[c + (tandy->crtc[1] << 3) + 8] = cols[0];
					else
						((uint32_t *)buffer32->line[tandy->displine])[c + (tandy->crtc[1] << 4) + 8] = cols[0];
				} else if ((tandy->mode & 0x12) == 0x12) {
					((uint32_t *)buffer32->line[tandy->displine])[c] = cgapal[0];
					if (tandy->mode & 1)
						((uint32_t *)buffer32->line[tandy->displine])[c + (tandy->crtc[1] << 3) + 8] = cgapal[0];
					else
						((uint32_t *)buffer32->line[tandy->displine])[c + (tandy->crtc[1] << 4) + 8] = cgapal[0];
				} else {
					((uint32_t *)buffer32->line[tandy->displine])[c] = cgapal[tandy->col & 15];
					if (tandy->mode & 1)
						((uint32_t *)buffer32->line[tandy->displine])[c + (tandy->crtc[1] << 3) + 8] = cgapal[tandy->col & 15];
					else
						((uint32_t *)buffer32->line[tandy->displine])[c + (tandy->crtc[1] << 4) + 8] = cgapal[tandy->col & 15];
				}
			}
...
//                                        if (!((ma^(crtc[15]|(crtc[14]<<8)))&0x3FFF)) printf("Cursor match! %04X\n",ma);
			...
			if (tandy->array[3] & 4) {
				if (tandy->mode & 1)
					hline(buffer32, 0, tandy->displine, (tandy->crtc[1] << 3) + 16, cgapal[tandy->array[2] & 0xf]);
				else
					hline(buffer32, 0, tandy->displine, (tandy->crtc[1] << 4) + 16, cgapal[tandy->array[2] & 0xf]);
			} else {
