...
//        pclog("pcjr OUT %04X %02X\n",addr,val);
	switch (addr) {
	case 0x3d4:pcjr->crtcreg = val & 0x1f;
		return;
	case 0x3d5:
...
//        if (addr!=0x3DA) pclog("pcjr IN %04X\n",addr);
	switch (addr) {
	case 0x3d4:return pcjr->crtcreg;
	case 0x3d5:return pcjr->crtc[pcjr->crtcreg];
	case 0x3da:pcjr->array_ff = 0;
		pcjr->stat ^= 0x10;
		return pcjr->stat;
	}
	return 0xFF;
