...
//        pclog("ati18800_out : %04X %02X  %04X:%04X\n", addr, val, CS,pc);
	...
	switch (addr) {
	case 0x1ce:ati18800->index = val;
		break;
	case 0x1cf:ati18800->regs[ati18800->index] = val;
		...
		case 0xbe:
			...
			break;
		case 0xb3:ati_eeprom_write(&ati18800->eeprom, val & 8, val & 2, val & 1);
			break;
		}
		break;
...
//        if (addr != 0x3da) pclog("ati18800_in : %04X ", addr);
	...
	switch (addr) {
	case 0x1ce:temp = ati18800->index;
		break;
	case 0x1cf:
		switch (ati18800->index) {
		case 0xb7:temp = ati18800->regs[ati18800->index] & ~8;
			if (ati_eeprom_read(&ati18800->eeprom))
				temp |= 8;
			break;

		default:temp = ati18800->regs[ati18800->index];
			break;
		}
		break;
	...
	case 0x3D4:temp = svga->crtcreg;
		break;
	case 0x3D5:temp = svga->crtc[svga->crtcreg];
		break;
...
static void ega_wonder_800_recalctimings(svga_t *svga) {
	...
	switch (clksel) {
	case 0:svga->clock = (cpuclock * (double)(1ull << 32)) / 25175000.0;
		break;
	case 1:svga->clock = (cpuclock * (double)(1ull << 32)) / 28322000.0;
		break;
	case 4:svga->clock = (cpuclock * (double)(1ull << 32)) / 14318181.0;
		break;
	case 5:svga->clock = (cpuclock * (double)(1ull << 32)) / 16257000.0;
		break;
	case 7:
