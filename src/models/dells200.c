...
static uint8_t dells200_in(uint16_t addr, void *p) {
	...
	switch (addr) {
	case 0xa4:ret = dells200.regs[0];
		break;
	case 0xa5:ret = dells200.regs[1];
		break;
	case 0xab:ret = dells200.regs[2];
		break;
	}
...
//        pclog("dells200_out: addr=%04x val=%02x %04x(%08x):%08x\n", addr, val, CS,cs,cpu_state.pc);
	...
	switch (addr) {
	case 0xa4:dells200.regs[0] = val;
		mem_a20_alt = val & 0x40;
		mem_a20_recalc();
		break;
	case 0xa5:dells200.regs[1] = val;
		if (val & 1)
