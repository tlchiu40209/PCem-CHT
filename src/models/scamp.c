...
static void recalc_mappings(void) {
	...
	for (; bank_nr < 2; bank_nr++) {
		...
		if (virt_base == 0) {
			...
			case BANK_NONE:fatal("Bank 0 is empty!\n");
				break;
...
//        pclog("shadow_control: addr=%08x size=%04x state=%i\n", addr, size, state);
	if (ems_enable)
		...
		switch (state) {
		case 0:mem_set_mem_state(addr, size, MEM_READ_EXTERNAL | MEM_WRITE_EXTERNAL);
			break;
		case 1:mem_set_mem_state(addr, size, MEM_READ_EXTERNAL | MEM_WRITE_INTERNAL);
			break;
		case 2:mem_set_mem_state(addr, size, MEM_READ_INTERNAL | MEM_WRITE_EXTERNAL);
			break;
		case 3:mem_set_mem_state(addr, size, MEM_READ_INTERNAL | MEM_WRITE_INTERNAL);
			break;
		}
	flushmmucache_nopc();
}
...
//        pclog("scamp_write: addr=%04x val=%02x\n", addr, val);
	...
	case 0xe8:scamp.ems_index = val & 0x1f;
...
//                                pclog("SCAMP CFG[%02x]=%02x\n", scamp.cfg_index, val);
				...
				case CFG_SLTPTR:recalc_sltptr();
					break;

				case CFG_RAMMAP:recalc_mappings();
					mem_mapping_disable(&ram_remapped_mapping);
					shadow_recalc();
					break;

				case CFG_EMSEN1:
				case CFG_EMSEN2:shadow_recalc();
					recalc_ems();
					break;
...
uint8_t scamp_read(uint16_t addr, void *p) {
	...
	switch (addr) {
	case 0x92:ret = scamp.port_92;
		break;

	case 0xe8:ret = scamp.ems_index | scamp.ems_autoinc;
		break;
...
//                pclog("Bank calc : %i = %08x\n", c ,addr);
		...
		case BANK_NONE:scamp.ram_mask[c] = 0;
			scamp.ram_interleaved[c] = 0;
			break;

		case BANK_256K:addr += 512 * 1024;
			scamp.ram_mask[c] = 0x1ff;
			scamp.row_phys_shift[c] = 10;
			scamp.ram_interleaved[c] = 0;
			break;

		case BANK_256K_INTERLEAVED:addr += 512 * 1024 * 2;
			scamp.ram_mask[c] = 0x1ff;
			scamp.row_phys_shift[c] = 10;
		...
		case BANK_1M:addr += 2048 * 1024;
			scamp.ram_mask[c] = 0x3ff;
			scamp.row_phys_shift[c] = 11;
			scamp.ram_interleaved[c] = 0;
			break;

		case BANK_1M_INTERLEAVED:addr += 2048 * 1024 * 2;
			scamp.ram_mask[c] = 0x3ff;
			scamp.row_phys_shift[c] = 11;
		...
		case BANK_4M:addr += 8192 * 1024;
			scamp.ram_mask[c] = 0x7ff;
			scamp.row_phys_shift[c] = 12;
			scamp.ram_interleaved[c] = 0;
			break;

		case BANK_4M_INTERLEAVED:addr += 8192 * 1024 * 2;
			scamp.ram_mask[c] = 0x7ff;
			scamp.row_phys_shift[c] = 12;
