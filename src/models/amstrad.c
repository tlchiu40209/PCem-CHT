...
uint8_t amstrad_read(uint16_t port, void *priv) {
	...
	switch (port) {
	case 0x378:return lpt1_read(port, NULL);
	case 0x379:return lpt1_read(port, NULL) | (amstrad_language & 7);
	case 0x37a:temp = lpt1_read(port, NULL) & 0x1f;
...
/* DIP switches 4,5: Initial video mode */
		...
		if (romset == ROM_PC1640) {
			...
			case AMSTRAD_NOLATCH:temp &= ~0x20;
				break;
			case AMSTRAD_SW9:temp &= ~0x20;
				break;
			case AMSTRAD_SW10:temp |= 0x20;
				break;
			}
		}
		return temp;
 ...
 * handled by the video card */
	...
	case 0xdead:return amstrad_dead;
	}
	return 0xff;
}
...
void amstrad_write(uint16_t port, uint8_t val, void *priv) {
	switch (port) {
	case 0x66:softresetx86();
		break;

	case 0x378:
	case 0x379:
	case 0x37a:lpt1_write(port, val, NULL);
		break;

	case 0xdead:amstrad_dead = val;
		break;
	}
}
