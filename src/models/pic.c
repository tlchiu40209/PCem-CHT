...
static uint8_t pic_elcrx_read(uint16_t addr, void *p) {
	...
	switch (addr) {
	case 0x4d0:temp = pic.level_sensitive;
		break;
	case 0x4d1:temp = pic2.level_sensitive;
		break;
	}
...
//        pclog("pic_elcrx_write: addr=%04x val=%02x\n", addr, val);
	switch (addr) {
	case 0x4d0:pic.level_sensitive = val & 0xf8;
		break;
	case 0x4d1:pic2.level_sensitive = val & 0xde;
		break;
	}
}
