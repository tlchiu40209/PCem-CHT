...
//                        pclog("Write UM8669F %02x [%02x] %02x\n", um8669f->cur_reg, um8669f->cur_device, val);
			...
			case REG_ENABLE:um8669f->dev[um8669f->cur_device].enable = val;
				break;
			case REG_ADDRLO:um8669f->dev[um8669f->cur_device].addr = (um8669f->dev[um8669f->cur_device].addr & 0xff00) | val;
				break;
			case REG_ADDRHI:um8669f->dev[um8669f->cur_device].addr = (um8669f->dev[um8669f->cur_device].addr & 0x00ff) | (val << 8);
				break;
			case REG_IRQ:um8669f->dev[um8669f->cur_device].irq = val;
				break;
			case REG_DMA:um8669f->dev[um8669f->cur_device].dma = val;
				break;
			}

			switch (um8669f->cur_device) {
			case DEV_FDC:fdc_remove();
				if (um8669f->dev[DEV_FDC].enable & 1)
					fdc_add();
				break;
			case DEV_COM1:serial1_remove();
				if (um8669f->dev[DEV_COM1].enable & 1)
					serial1_set(um8669f->dev[DEV_COM1].addr, um8669f->dev[DEV_COM1].irq);
				break;
			case DEV_COM2:serial2_remove();
				if (um8669f->dev[DEV_COM2].enable & 1)
					serial2_set(um8669f->dev[DEV_COM2].addr, um8669f->dev[DEV_COM2].irq);
				break;
			case DEV_LPT1:lpt1_remove();
				lpt2_remove();
...
//        pclog("Read UM8669F %02x\n", um8669f->cur_reg);
	...
	switch (um8669f->cur_reg) {
	case REG_DEVICE:return um8669f->cur_device;
	case REG_ENABLE:return um8669f->dev[um8669f->cur_device].enable;
	case REG_ADDRLO:return um8669f->dev[um8669f->cur_device].addr & 0xff;
	case REG_ADDRHI:return um8669f->dev[um8669f->cur_device].addr >> 8;
	case REG_IRQ:return um8669f->dev[um8669f->cur_device].irq;
	case REG_DMA:return um8669f->dev[um8669f->cur_device].dma;
	}
