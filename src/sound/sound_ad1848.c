...
uint8_t ad1848_read(uint16_t addr, void *p) {
	...
	case 0: /*Index*/
		...
		break;
	case 1:temp = ad1848->regs[ad1848->index];
		break;
	case 2:temp = ad1848->status;
		break;
	}
...
//                pclog("ad1848_write - addr %04X val %02X  %04X(%08X):%08X\n", addr, val, CS, cs, cpu_state.pc);
	...
	case 1:
		switch (ad1848->index) {
		case 8:freq = (val & 1) ? 16934400 : 24576000;
			switch ((val >> 1) & 7) {
			case 0:freq /= 3072;
				break;
			case 1:freq /= 1536;
		...
		case 12:ad1848->regs[12] = ((ad1848->regs[12] & 0x0f) + (val & 0xf0)) | 0x80;
			return;

		case 14:ad1848->count = ad1848->regs[15] | (val << 8);
			break;
