	...
	for (c = 0; c < joystick_get_axis_count(joystick_config_type); c++) {
		...
		if (joystick) {
			...
			for (d = 0; d < plat_joystick_state[joystick - 1].nr_povs; d++) {
				...
				sprintf(s, "%s (X 軸)", plat_joystick_state[joystick - 1].pov[d].name);
				wx_sendmessage(h, WX_CB_ADDSTRING, 0, (LONG_PARAM)s);
				sprintf(s, "%s (Y 軸)", plat_joystick_state[joystick - 1].pov[d].name);
				wx_sendmessage(h, WX_CB_ADDSTRING, 0, (LONG_PARAM)s);
			}
...
//                        if (HIWORD(wParam) == CBN_SELCHANGE)
		...
		case wxID_CANCEL:wx_enddialog(hdlg, 0);
			return TRUE;
		}
		break;
	}
...
//	dialog.SetWindowStyle(wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER);
	...
	wxString wxDevice("裝置",wxConvUTF8);
	sizer->Add(new wxStaticText(&dialog, id++, wxDevice), 0, wxALL | wxALIGN_CENTER_VERTICAL, 5);
	...
	for (c = 0; c < joystick_get_axis_count(type); c++) {
		sprintf(s, "Axis %i:", c);
		wxString wxAxis(s, wxConvUTF8);
		sizer->Add(new wxStaticText(&dialog, id++, wxAxis), 0, wxALL | wxALIGN_CENTER_VERTICAL, 5);
		wxBoxSizer *comboSizer = new wxBoxSizer(wxHORIZONTAL);
		sizer->Add(comboSizer, 1, wxEXPAND, 5);
		wxComboBox *cb = new wxComboBox(&dialog, id++);
		cb->SetEditable(false);
	...
	for (c = 0; c < joystick_get_button_count(type); c++) {
		sprintf(s, "按鍵 %i:", c);
		wxString wxButton(s, wxConvUTF8);
		sizer->Add(new wxStaticText(&dialog, id++, wxButton), 0, wxALL | wxALIGN_CENTER_VERTICAL, 5);
		wxBoxSizer *comboSizer = new wxBoxSizer(wxHORIZONTAL);
		sizer->Add(comboSizer, 1, wxEXPAND, 5);
		wxComboBox *cb = new wxComboBox(&dialog, id++);
		cb->SetEditable(false);
