...
void wx_initmenu() {
	...
	for (c = 'A'; c <= 'Z'; c++) {
		...
		if (GetDriveTypeA(s) == DRIVE_CDROM) {
			sprintf(s, "本機 CD/DVD 光碟機 (%c:)", c);
			wx_appendmenu(cdrom_submenu, IDM_CDROM_REAL + c, s, wxITEM_RADIO);
		}
	}
#elif __linux__
	wx_appendmenu(cdrom_submenu, IDM_CDROM_REAL + 1, "本機 CD/DVD 光碟機 (/dev/cdrom)", wxITEM_RADIO);
#elif __APPLE__
	int c;
	...
	for (c = 1; c < 99; c++)
	{
		...
		if (fd > 0)
		{
			...
			sprintf(name, "本機 CD/DVD 光碟機 (/dev/disk%i)", c);
...
int start_emulation(void *params) {
	...
	if (!video_card_available(video_old_to_new(gfxcard))) {
		if (romset != -1)
			wx_messagebox(ghwnd,
				      "找不到您所選的顯示卡之對應的 V-BIOS ROM 檔案\n預設將使用其他可用的 V-BIOS ROM。",
				      "PCem 錯誤", WX_MB_OK);
...
int getfile(void *hwnd, char *f, char *fn) {
	int ret = wx_filedialog(hwnd, "開啟檔案", fn, f, 0, 1, openfilestring);
#ifdef __APPLE__
...
int getsfile(void *hwnd, char *f, char *fn, char *dir, char *ext) {
	int ret = wx_filedialog(hwnd, "儲存檔案", dir, f, ext, 0, openfilestring);
#ifdef __APPLE__
	window_doreset = 1;
#endif
	return ret;
...
int wx_handle_command(void *hwnd, int wParam, int checked) {
	...
	if (ID_IS("IDM_STATUS")) {
		...
		if (!getfile(hwnd,
			     "軟碟映像檔 (*.img;*.ima;*.fdi)|*.img;*.ima;*.fdi|所有檔案 (*.*)|*.*",
			     discfns[0])) {
			disc_close(0);
