...
static int config_selection_dlgproc(void* hdlg, int message, INT_PARAM wParam, LONG_PARAM lParam)
{
        ...
        //                        pause = 0;
                        ...
                        else if (wParam == WX_ID("IDC_NEW"))
                        {
                                ...
                                while (!done)
                                {
                                        if (wx_textentrydialog(hdlg, "請輸入名稱", "新增虛擬機器", name, 1, 64, (LONG_PARAM)name))
                                        {
                                                if(validate_config_name(name) != NULL) 
                                                {
                                                        wx_simple_messagebox("指定名稱不正確", "指定名稱不能包含這些字元: /, <, >, :, \", \\, |, ?, *");
                                                } 
                                                else 
                                                {
                                                        char cfg[512];
                                                        ...
                                                        if (!wx_file_exists(cfg))
                                                        {
                                                                ...
                                                                wx_simple_messagebox("指定名稱已存在", "其他的虛擬機器已經使用這個名稱。");
                                                }
                                        }
                                        else
                                                done = 1;
                        ...
                        else if (wParam == WX_ID("IDC_RENAME"))
                        {
                                ...
                                if (c >= 0)
                                {
                                        ...
                                        while (!done)
                                        {
                                                if (wx_textentrydialog(hdlg, "請輸入名稱:", "重新命名虛擬機", name, 1, 64, (LONG_PARAM)name))
                                                {
                                                        char old_path[512];
                                                        char new_path[512];
                        ...
                        else if (wParam == WX_ID("IDC_COPY"))
                        {
                                ...
                                if (c >= 0)
                                {
                                        ...
                                        while (!done)
                                        {
                                                if (wx_textentrydialog(hdlg, "請輸入名稱:", "複製虛擬機", name, 1, 64, (LONG_PARAM)name))
                                                {
                                                        char old_path[512];
                                                        char new_path[512];
