...
int confirm()
{
        if (emulation_state != EMULATION_STOPPED) {
                return wx_messagebox(NULL, "這會重設PCem!\n您確定要繼續嗎?", "PCem", WX_MB_OKCANCEL) == WX_IDOK;
        }
        return 1;
}
...
int wx_messagebox(void* window, const char* message, const char* title = NULL, int style = 5)
{
        ...
        return wxMessageBox(wxMessage, wxTitle, style | wxCENTRE | wxSTAY_ON_TOP, (wxWindow*) window);
}
...
void wx_simple_messagebox(const char* title, const char *format, ...)
{
		...
		wxString wxMessage(message,wxConvUTF8);
		wxString wxTitle(title,wxConvUTF8);
        wx_messagebox(0, wxMessage, wxTitle, WX_MB_OK);
}
...
int wx_textentrydialog(void* window, const char* message, const char* title, const char* value, unsigned int min_length, unsigned int max_length, LONG_PARAM result)
{
        while (1)
        {
                ...
                wxTextEntryDialog dlg((wxWindow*)window, wxMessage, wxTitle, wxValue);
                if (max_length > 0)
...
{
		wxString wxTitle(title,wxConvUTF8);
		wxString wxExts(extensions, wxConvUTF8);
        wxFileDialog dlg((wxWindow*) window, wxTitle, "", path, wxExts,
...
{
		wxString wxTitle(title,wxConvUTF8);
        ((wxMenu*) sub_menu)->Append(id, wxTitle, wxEmptyString, type);
}
...
void wx_setdlgitemtext(void* window, int id, char* str)
{
        ...
        if (w->GetClassInfo()->IsKindOf(CLASSINFO(wxTextCtrl)))
                ((wxTextCtrl*) w)->SetValue(wxStr);
        else
                ((wxStaticText*) w)->SetLabel(wxStr);
}
...
int wx_sendmessage(void* window, int type, LONG_PARAM param1, LONG_PARAM param2) //
{
		...
		{
				wxString wxParam2cba((char*)param2,wxConvUTF8);
                ((wxComboBox*) window)->Append(wxParam2cba);
                break;
		}
		...
		{
                strcpy((char*) param2, ((wxComboBox*) window)->GetString(param1).mb_str()); // 讀取回來可能沒有問題
                break;
		}
        ...
        case WX_WM_SETTEXT:
        {
				wxString wxParam2wmSet((char*)param2,wxConvUTF8);
                if (((wxWindow*) window)->GetClassInfo()->IsKindOf(CLASSINFO(wxTextCtrl)))
                        ((wxTextCtrl*) window)->SetValue(wxParam2wmSet);
                else
                        ((wxStaticText*) window)->SetLabel(wxParam2wmSet);
        }
                break;
        case WX_WM_GETTEXT:
		{
        ...
        {
				wxString wxParam2lbIns((char*)param2,wxConvUTF8);
                ((wxListBox*) window)->Insert(wxParam2lbIns, param1);
                break;
        }
        ...
        {
				wxString wxParam2ChbSet((char*)param2,wxConvUTF8);
                ((wxChoicebook*)window)->SetPageText(param1, wxParam2ChbSet);
                break;
        }
        case WX_CHB_ADDPAGE:
        {
				wxString wxParam2ChbAdd((char*)param2,wxConvUTF8);
                ((wxChoicebook*)window)->AddPage((wxWindow *)param1, wxParam2ChbAdd);
                break;
        }
...
int wx_progressdialog(void* window, const char* title, const char* message, WX_CALLBACK callback, void* data, int range, volatile int *pos)
{
        ...
        wxProgressDialog dlg(wxTitle, wxMessage, range, (wxWindow*)window, wxPD_APP_MODAL | wxPD_SMOOTH | wxPD_AUTO_HIDE);
        while (pdata.active)
        {
                dlg.Update(*pos);
                wxMilliSleep(50);
