...
//      dialog.SetWindowStyle(wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER);
	...
	if (glsl->num_parameters > 0) {
		...
		wxString wxNoConfig("沒有可用的設定", wxConvUTF8);
		winSizer->Add(new wxStaticText(win, wxID_ANY, wxNoConfig), 0, wxALL);
	}

	wxBoxSizer *buttonSizer = new wxBoxSizer(wxHORIZONTAL);
	wxString wxApply("套用", wxConvUTF8);
	wxString wxReset("重設", wxConvUTF8);
	buttonSizer->Add(new wxButton(&dialog, ID_APPLY, wxApply));
	buttonSizer->Add(new wxButton(&dialog, wxID_OK));
	buttonSizer->Add(new wxButton(&dialog, wxID_CANCEL));
	buttonSizer->Add(new wxButton(&dialog, ID_RESET, wxReset));
