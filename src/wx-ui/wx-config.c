...
static void recalc_vid_list(void *hdlg, int model, int force_builtin_video) {
	...
	if (model_has_fixed_gfx(model)) {
		wx_sendmessage(h, WX_CB_ADDSTRING, 0, (LONG_PARAM)"內建顯示");
		wx_sendmessage(h, WX_CB_SETCURSEL, 0, 0);
		wx_enablewindow(h, FALSE);
...
static void recalc_hdd_list(void *hdlg, int model, int use_selected_hdd, int force_ide) {
	...
	if (hdd_controller_is_scsi(hdd_names[hdd_type])) {
		...
		for (c = 0; c < 4; c++) {
			...
			page = wx_getdlgitem(hdlg, WX_ID(s));
			wx_sendmessage(page, WX_REPARENT, (LONG_PARAM)h, 0);
			sprintf(s, "硬碟機 %i %s %s (%c:)", c, (c & 2) ? "次要(第二組)IDE" : "主要(第一組)IDE", (c & 1) ? "Slave" : "Master", 'C'+c);
			wx_sendmessage(h, WX_CHB_SETPAGETEXT, c, (LONG_PARAM)s);
		}
		for (c = 6; c >= 4; c--) {
		...
		for (c = 0; c < 2; c++) {
			...
			page = wx_getdlgitem(hdlg, WX_ID(s));
			wx_sendmessage(page, WX_REPARENT, (LONG_PARAM)h, 0);
			sprintf(s, "硬碟機 %i %s (%c:)", c, (c & 1) ? "次要" : "主要", 'C'+c);
			wx_sendmessage(h, WX_CHB_SETPAGETEXT, c, (LONG_PARAM)s);
		}
		for (c = 6; c >= 2; c--) {
			wx_sendmessage(h, WX_CHB_REMOVEPAGE, c, 0);
...
int config_dlgproc(void *hdlg, int message, INT_PARAM wParam, LONG_PARAM lParam) {
	...
	case WX_INITDIALOG: {
		...
		h = wx_getdlgitem(hdlg, WX_ID("IDC_COMBOSPD"));
		wx_sendmessage(h, WX_CB_ADDSTRING, 0, (LONG_PARAM) "預設");
		wx_sendmessage(h, WX_CB_ADDSTRING, 0, (LONG_PARAM) "8 位元");
		wx_sendmessage(h, WX_CB_ADDSTRING, 0, (LONG_PARAM) "低速 16 位元");
		wx_sendmessage(h, WX_CB_ADDSTRING, 0, (LONG_PARAM) "高速 16 位元");
		wx_sendmessage(h, WX_CB_ADDSTRING, 0, (LONG_PARAM) "低速 VLB/PCI");
		wx_sendmessage(h, WX_CB_ADDSTRING, 0, (LONG_PARAM) "中速 VLB/PCI");
		wx_sendmessage(h, WX_CB_ADDSTRING, 0, (LONG_PARAM) "高速 VLB/PCI");
		...
		h = wx_getdlgitem(hdlg, WX_ID("IDC_COMBOWS"));
		wx_sendmessage(h, WX_CB_ADDSTRING, 0, (LONG_PARAM) "系統預設");
		...
		for (c = 0; c < 7; c++) {
			...
			h = wx_getdlgitem(hdlg, WX_ID(s));
			wx_sendmessage(h, WX_CB_ADDSTRING, 0, (LONG_PARAM)"硬碟機");
			wx_sendmessage(h, WX_CB_ADDSTRING, 0, (LONG_PARAM)"CD 光碟機");
			wx_sendmessage(h, WX_CB_ADDSTRING, 0, (LONG_PARAM)"ZIP 碟機");
			if (cdrom_channel == c)
...
static int hdnew_dlgproc(void *hdlg, int message, INT_PARAM wParam, LONG_PARAM lParam) {
	...
	switch (message) {
	case WX_INITDIALOG:h = wx_getdlgitem(hdlg, WX_ID("IDC_EDIT1"));
		sprintf(s, "%i", 63);
...
//                hd_type = 0;
		...
		wx_sendmessage(h, WX_CB_ADDSTRING, 0, (LONG_PARAM)"自訂類型");
		for (c = 1; c <= 46; c++) {
			sprintf(s,
				"類型 %02i : 磁住=%i, 讀頭=%i, 容量=%iMB",
				c,
				hd_types[c - 1].cylinders,
				hd_types[c - 1].heads,
		...
		wx_sendmessage(h, WX_CB_ADDSTRING, 0, (LONG_PARAM)"原生映像檔 (.img)");
		wx_sendmessage(h, WX_CB_ADDSTRING, 0, (LONG_PARAM)"固定大小 VHD (.vhd)");
		wx_sendmessage(h, WX_CB_ADDSTRING, 0, (LONG_PARAM)"動態大小 VHD (.vhd)");
		wx_sendmessage(h, WX_CB_ADDSTRING, 0, (LONG_PARAM)"差異 VHD (.vhd)");
		wx_sendmessage(h, WX_CB_SETCURSEL, 0, 0);

		h = wx_getdlgitem(hdlg, WX_ID("IDC_HDBLOCKSIZE"));
		wx_sendmessage(h, WX_CB_ADDSTRING, 0, (LONG_PARAM)"大儲存區塊 (2 MB)");
		wx_sendmessage(h, WX_CB_ADDSTRING, 0, (LONG_PARAM)"小儲存區塊 (512 KB)");
		wx_sendmessage(h, WX_CB_SETCURSEL, 0, 0);
	...
	case WX_COMMAND:
		if (wParam == wxID_OK) {
			...
			if (!hd_new_name[0]) {
				wx_messagebox(hdlg,"請輸入正確的檔案名稱","PCem 錯誤",WX_MB_OK);
				return TRUE;
			}
			...
			if (hd_new_spt > 63) {
				wx_messagebox(hdlg, "您指定給該磁碟的磁區過多 (最多 63)", "PCem 錯誤", WX_MB_OK);
				return TRUE;
			}
			if (hd_new_hpc > 16) {
				wx_messagebox(hdlg, "您指定給該磁碟的讀頭過多 (最多 16)", "PCem 錯誤", WX_MB_OK);
				return TRUE;
			}
			if (hd_new_cyl > MAX_CYLINDERS) {
				char s[256];
				sprintf(s, "您指定給該磁碟的磁住過多 (最多 %i)", MAX_CYLINDERS);
				wx_messagebox(hdlg, s, "PCem 錯誤", WX_MB_OK);
				return TRUE;
			}
			...
			if (hd_format == 0) /* Raw .img */
			{
				...
				if (!f) {
					wx_messagebox(hdlg, "無法開啟並寫入該檔案", "PCem 錯誤", WX_MB_OK);
					return TRUE;
				}
				wx_progressdialog(hdlg, "PCem", "建立磁碟映像中，請稍後...", create_drive_raw, f, hd_new_cyl * hd_new_hpc * hd_new_spt, &create_drive_pos);
			} else if (hd_format == 1) /* Fixed VHD */
			{
				if (!wx_progressdialog(hdlg, "PCem", "建立磁碟映像中，請稍後...", create_drive_vhd_fixed, NULL, hd_new_cyl * hd_new_hpc * hd_new_spt, &create_drive_pos)) {
					wx_messagebox(hdlg, "無法建立 VHD", "PCem 錯誤", WX_MB_OK);
					return TRUE;
				}
			...
			{
				if (!getfilewithcaption(hdlg, "VHD 檔案 (*.vhd)|*.vhd|所有檔案 (*.*)|*.*", "", "選擇參照 VHD")) {
					if (!create_drive_vhd_diff(openfilestring, hd_blocksize)) {
						wx_messagebox(hdlg, "無法建立 VHD", "PCem 錯誤", WX_MB_OK);
						return TRUE;
					}
				}
				wx_messagebox(hdlg, "差異 VHD 映像已建立成功。\n\n"
						    "警告: 若您上層參照的 VHD 仍有其他關連的差異 VHD 映像，請勿參照 VHD 上的內容", "PCem", WX_MB_OK);
			}
			if (hd_format != 3) {
				wx_messagebox(hdlg, "磁碟映像已建立，請記得為其做磁碟分割與進行格式化。", "PCem", WX_MB_OK);
			}
			...
			hd_format = wx_sendmessage(h, WX_CB_GETCURSEL, 0, 0);
			if (!getsfile(hdlg,
				      hd_format == 0 ? "原生映像檔 (*.img)|*.img|All files (*.*)|*.*" : "VHD 檔 (*.vhd)|*.vhd|所有檔案 (*.*)|*.*",
				      "",
				      NULL,
				      hd_format == 0 ? "img" : "vhd")) {
				h = wx_getdlgitem(hdlg, WX_ID("IDC_EDITC"));
			...
			{
				h = wx_getdlgitem(hdlg, WX_ID("IDC_EDIT1"));
				wx_sendmessage(h, WX_WM_SETTEXT, 0, (LONG_PARAM)"(使用參照)");
			...
			{
				h = wx_getdlgitem(hdlg, WX_ID("IDC_HDBLOCKSIZELABEL"));
				wx_sendmessage(h, WX_WM_SETTEXT, 0, (LONG_PARAM)"區塊大小:");
				h = wx_getdlgitem(hdlg, WX_ID("IDC_HDBLOCKSIZE"));
...
static int hdsize_dlgproc(void *hdlg, int message, INT_PARAM wParam, LONG_PARAM lParam) {
	...
	switch (message) {
	case WX_INITDIALOG:h = wx_getdlgitem(hdlg, WX_ID("IDC_HDTYPE"));
		wx_sendmessage(h, WX_CB_ADDSTRING, 0, (LONG_PARAM)"自訂類型");
		for (c = 1; c <= 46; c++) {
			sprintf(s,
				"類型 %02i : 磁住=%i, 讀頭=%i, 容量=%iMB",
...
static int hd_file(void *hdlg, int drive) {
	if (!getfile(hdlg, "磁碟映像檔 (*.img;*.vhd)|*.img;*.vhd|所有檔案 (*.*)|*.*", "")) {
		off64_t sz;
		FILE *f = fopen64(openfilestring, "rb");
		if (!f) {
			wx_messagebox(hdlg, "無法開啟並讀取檔案", "PCem 錯誤", WX_MB_OK);
			return TRUE;
		}
		...
		if (mvhd_file_is_vhd(f)) {
			...
			if (vhd == NULL) {
				...
				const char ts_warning[] =
					"警告: 差異 VHD 與其參照 VHD 的時間戳記不相符!\n\n"
					"這表示在差異 VHD 建立後，其參照 VHD 遭到變更。\n\n"
					"如果您曾有移動或複製過參照 VHD，或者您是使用 DiskPart 指令建立 VHD 也會造成這個現象。\n\n"
					"如果您曾有過上述操作，您想要自動修復時間戳記問題嗎?";
				int wx_res = wx_messagebox(hdlg, ts_warning, "PCem 錯誤", WX_MB_NODEFAULT);
				if (wx_res == WX_MB_YES) {
					int ts_res = mvhd_diff_update_par_timestamp(vhd, &vhdError);
					if (ts_res != 0) {
						wx_messagebox(hdlg, "無法修復 VHD 戳記問題","PCem 錯誤", WX_MB_OK);
						mvhd_close(vhd);
						return TRUE;
					}
				} else {
