...
//                hd_type = 0;
                ...
                for (c = 1; c <= 46; c++)
                {
                        sprintf(s, "類型 %02i : 磁住=%i, 讀頭=%i, 容量=%iMB", c, hd_types[c-1].cylinders, hd_types[c-1].heads, (hd_types[c-1].cylinders * hd_types[c-1].heads * 17 * 512) / (1024 * 1024));
                        wx_sendmessage(h, WX_CB_ADDSTRING, 0, (LONG_PARAM)s);
                }
                wx_sendmessage(h, WX_CB_SETCURSEL, 0, 0);
...
static int hd_file(void *hdlg, int drive)
{
        if (!getfile(hdlg, "磁碟映像檔 (*.img;*.vhd)|*.img;*.vhd|所有檔案 (*.*)|*.*", ""))
        {
                ...
                if (mvhd_file_is_vhd(f))
                {
                        ...
                        if (vhd == NULL)
                        {
                                wx_messagebox(hdlg,mvhd_strerr(vhdError),"PCem 錯誤",WX_MB_OK);
                                return TRUE;
                        }
