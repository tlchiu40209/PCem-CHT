...
void StatusPane::Render(wxDC &dc) {
	...
	if (emulation_state != EMULATION_STOPPED) {
		...
		if (show_speed_history) {
			...
			dc.DrawLine(speedGraphX,
				    speedGraphY + speedGraphBorder + speedGraphHeight - 100,
				    speedGraphX + speedGraphWidth,
				    speedGraphY + speedGraphHeight + speedGraphBorder - 100); // 100
			dc.DrawLine(speedGraphX,
				    speedGraphY + speedGraphBorder + speedGraphHeight - 125,
				    speedGraphX + speedGraphWidth,
...
int wx_window_x = 0;
int wx_window_y = 0;
wxString pcEmVmTitle("PCem 虛擬機", wxConvUTF8);

StatusFrame::StatusFrame(wxWindow *parent) :
	wxFrame(parent, STATUS_WINDOW_ID, pcEmVmTitle, wxPoint(0, 0), wxSize(DEFAULT_WINDOW_WIDTH, 200), wxCAPTION | wxCLOSE_BOX) {
...
void StatusFrame::OnCommand(wxCommandEvent &event) {
	if (event.GetId() == XRCID("TOOLBAR_RUN")) {
		...
		if (confirm_on_reset_machine) {
			...
			wxString goingToReset("強制重新啟動虛擬機嗎?", wxConvUTF8);
			wxString dontAsk("別再詢問我", wxConvUTF8);
			dlg.FindWindow("IDC_CONFIRM_LABEL")->SetLabel(goingToReset);
			dlg.FindWindow("IDC_CONFIRM_REMEMBER")->SetLabel(dontAsk);
			dlg.Fit();
			ret = dlg.ShowModal();
			if (ret == wxID_OK)
...
//                hdconf_open(this);
		...
		wxString confrirmDiagReset("確認對話方塊已重設", wxConvUTF8);
		wxMessageBox(confrirmDiagReset, "PCem");
