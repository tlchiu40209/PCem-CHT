...
int deviceconfig_dlgproc(void *hdlg, int message, INT_PARAM wParam,
	...
	case WX_INITDIALOG: {
		...
		while (config->type != -1) {
			...
			switch (config->type) {
			case CONFIG_BINARY:h = wx_getdlgitem(hdlg, id);
			...
			case CONFIG_SELECTION:h = wx_getdlgitem(hdlg, id + 1);
				val_int = config_get_int(CFG_MACHINE, config_device->name, config->name, config->default_int);
			...
			case CONFIG_MIDI:num = midi_get_num_devs();
				if (num > 0) {
...
//	dialog.SetWindowStyle(wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER);
	...
	while (config->type != -1) {
		switch (config->type) {
		case CONFIG_BINARY:sizer->Add(0, 0, 1, wxEXPAND, 5);
