...
*/
	...
	void Channel::WriteC0(const Chip *chip, Bit8u val) {
		...
		if (chip->opl3Active) {
			...
			if ((chip->reg104 & fourMask) & 0x3f) {
				...
				switch (synth) {
				case 0: chan0->synthHandler = &Channel::BlockTemplate<sm3FMFM>;
					break;
				case 1: chan0->synthHandler = &Channel::BlockTemplate<sm3AMFM>;
					break;
				case 2: chan0->synthHandler = &Channel::BlockTemplate<sm3FMAM>;
					break;
				case 3: chan0->synthHandler = &Channel::BlockTemplate<sm3AMAM>;
					break;
				}
	...
	Channel *Channel::BlockTemplate(Chip *chip, Bit32u samples, Bit32s *output) {
		...
		for (Bitu i = 0; i < samples; i++) {
			...
			case sm3AMFM:
			case sm3FMAM:
			case sm3AMAM: output[i * 2 + 0] += sample & maskLeft;
		...
		case sm2FM:
		case sm3AM:
		case sm3FM: return (this + 1);
		case sm3FMFM:
		case sm3AMFM:
		case sm3FMAM:
		case sm3AMAM: return (this + 2);
		case sm2Percussion:
		case sm3Percussion: return (this + 3);
		}
		return 0;
	}
...
#define REGCHAN(_FUNC_)                                                                                                                                \
	...
	void Chip::WriteReg(Bit32u reg, Bit8u val) {
		...
		case 0x10 >> 4: break;
		case 0x20 >> 4:
		case 0x30 >> 4: REGOP(Write20);
			break;
		case 0x40 >> 4:
		case 0x50 >> 4: REGOP(Write40);
			break;
		case 0x60 >> 4:
		case 0x70 >> 4: REGOP(Write60);
			break;
		case 0x80 >> 4:
		case 0x90 >> 4: REGOP(Write80);
			break;
		case 0xa0 >> 4: REGCHAN(WriteA0);
			break;
		case 0xb0 >> 4:
			if (reg == 0xbd) {
				WriteBD(val);
			...
			break;
		case 0xc0 >> 4: REGCHAN(WriteC0);
		case 0xd0 >> 4: break;
		case 0xe0 >> 4:
		case 0xf0 >> 4: REGOP(WriteE0);
			break;
		}
	}
